'use strict';
const DATA=window.FALLGUARD_DATA, rows=DATA.aggregates, edge=DATA.edge;
...
function resultTable(target,data,metric){$(target).innerHTML=publicTable(['Model / variant',metricNames[metric]||metric,'Sample SD','n'],data.map(d=>[d.name,value(d.mean),value(d.sd),d.n]),'Exact mean and sample standard deviation');}
function dotChart(target,data,metric){
 const W=1000,left=250,right=145,top=38,rowH=44,H=top+data.length*rowH+50,rate=metric.includes('per_hour');
 const max=rate?Math.ceil(Math.max(...data.map(d=>d.mean+d.sd))/50)*50:1;
 const x=v=>left+Math.max(0,Math.min(max,v))/max*(W-left-right);
 let svg=`<svg viewBox="0 0 ${W} ${H}" role="img" aria-label="${esc(metricNames[metric])}: means and sample standard deviations"><title>${esc(metricNames[metric])}</title><desc>Each row shows a mean, sample standard deviation, and sample count. Exact values are in the table below.</desc>`;
 for(let i=0;i<=5;i++){const v=max*i/5,X=x(v);svg+=`<line class="grid" x1="${X}" x2="${X}" y1="${top-15}" y2="${H-45}"/><text class="tick" x="${X}" y="${H-22}" text-anchor="middle">${rate?v.toFixed(0):v.toFixed(1)}</text>`;}
 data.forEach((d,i)=>{const y=top+i*rowH,color=d.featured?'#007e74':'#7c8f91',label=`${d.name}: ${value(d.mean)} ± ${value(d.sd)} (n=${d.n})`;
 svg+=`<g class="chart-row" tabindex="0" data-tip="${esc(label)}" aria-label="${esc(label)}"><rect class="row-bg" x="0" y="${y-17}" width="${W}" height="${rowH}"/><text x="${left-20}" y="${y+7}" text-anchor="end">${esc(d.name)}</text><line class="whisker" stroke="${color}" x1="${x(d.mean-d.sd)}" x2="${x(d.mean+d.sd)}" y1="${y}" y2="${y}"/><line stroke="${color}" x1="${x(d.mean-d.sd)}" x2="${x(d.mean-d.sd)}" y1="${y-5}" y2="${y+5}"/><line stroke="${color}" x1="${x(d.mean+d.sd)}" x2="${x(d.mean+d.sd)}" y1="${y-5}" y2="${y+5}"/><circle cx="${x(d.mean)}" cy="${y}" r="${d.featured?6:5}" fill="${color}"/><text x="${W-right+23}" y="${y+5}">${value(d.mean,rate?1:4)}</text></g>`;
 });svg+=`<text class="tick" x="${left}" y="16">${rate?'Lower is better':'Higher is better'} · whiskers = sample SD</text></svg>`;$(target).innerHTML=svg;
}
function updateBaseline(){const m=$('mainMetric').value;const d=['tcn','te','tcnte','unimamba','masked_bimamba'].map(k=>({...lookup('gmdcsa24_main_v1',k,m),name:modelNames[k],featured:k==='masked_bimamba'}));dotChart('baselineChart',d,m);resultTable('baselineTable',d,m);}
...
function updateRobustness(){
 ...
 ['masked_bimamba','tcnte'].forEach((model,index)=>{
  ...
  const band=[...d.map(r=>`${x(r.s)},${y(r.mean+r.sd)}`),...d.slice().reverse().map(r=>`${x(r.s)},${y(r.mean-r.sd)}`)].join(' ');
...
document.addEventListener('pointerleave',hideRuler);$('highlightToggle').addEventListener('change',hideRuler);
const tooltip=$('chartTooltip');document.addEventListener('pointerover',e=>{const target=e.target.closest('[data-tip]');if(target){tooltip.textContent=target.dataset.tip;tooltip.hidden=false;}});document.addEventListener('pointermove',e=>{if(!e.target.closest('[data-tip]')){tooltip.hidden=true;return}tooltip.style.left=Math.max(8,Math.min(innerWidth-315,e.clientX+14))+'px';tooltip.style.top=Math.max(8,Math.min(innerHeight-85,e.clientY+15))+'px';});document.addEventListener('focusin',e=>{const target=e.target.closest('[data-tip]');if(target){const r=target.getBoundingClientRect();tooltip.textContent=target.dataset.tip;tooltip.hidden=false;tooltip.style.left=Math.max(8,Math.min(innerWidth-315,r.left+30))+'px';tooltip.style.top=Math.max(8,Math.min(innerHeight-85,r.bottom+4))+'px';}});document.addEventListener('focusout',()=>tooltip.hidden=true);
